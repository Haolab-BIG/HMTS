...
#java -jar ${trimmomatic} SE -phred33 -threads 10 /mnt1/2.NAS2024/wutan/4.translocation/Degron/5.DOX/2.trim/decompress/test.fastq /mnt1/2.NAS2024/wutan/4.translocation/Degron/5.DOX/2.trim/decompress/test.trim.fastq ILLUMINACLIP:/mnt1/2.NAS2024/wutan/4.translocation/Degron/5.DOX/2.trim/decompress/Read1_bait_adapter.txt:2:30:7 CROP:150 MINLEN:20
#decompressdir=/mnt1/2.NAS2024/wutan/4.translocation/Degron/5.DOX/2.trim/decompress
#cutadapt -a file:/mnt1/2.NAS2024/wutan/4.translocation/Degron/5.DOX/2.trim/decompress/Read1_bait_adapter.txt -j 10 -e 0.1 -m 20 -o ${decompressdir}/test.trimed.fastq ${decompressdir}/test.fastq
...
starres=/mnt1/2.NAS2024/wutan/4.translocation/Degron/5.DOX/10.star
cd ${starres}
...
for fileName in wm-DOX1_S4_L001 wm-DOX2_S5_L001;do
	...
	awk 'BEGIN{FS=OFS="\t"}NR==FNR{if(NR%4==1){name=$1}else if(NR%4==2){A[name]=$1}else if(NR%4==3){B[name]=$1}else if(NR%4==0){C[name]=$1}}NR>FNR{if(A[$1]!=""){print $1"\n"A[$1]"\n"B[$1]"\n"C[$1]}}' ${trimdir}/${fileName}_R1.filter2.5.fastq ${fileName}_R1_filter.name > ${trimdir}/${fileName}_R1_001.filter3.fastq
	awk 'BEGIN{FS=OFS="\t"}NR==FNR{if(NR%4==1){name=$1}else if(NR%4==2){A[name]=$1}else if(NR%4==3){B[name]=$1}else if(NR%4==0){C[name]=$1}}NR>FNR{if(A[$1]!=""){print $1"\n"A[$1]"\n"B[$1]"\n"C[$1]}}' ${trimdir}/${fileName}_R2.filter2.5.fastq ${fileName}_R2_filter.name > ${trimdir}/${fileName}_R2_001.filter3.fastq
